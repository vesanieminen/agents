...
<style>
:root{--ground:#F4F6F7;--surface:#fff;--surface-2:#EAEFEF;--ink:#111819;--muted:#5C6B6D;--rule:#DBE2E2;--accent:#0F6E68;
 --working:#2A63C7;--needs_you:#B4650B;--errored:#B23A2E;--done:#3F7A52;--stale:#6E5BB0;--closed:#6B7280}
@media (prefers-color-scheme:dark){:root{--ground:#0D1314;--surface:#141C1D;--surface-2:#1B2526;--ink:#E8EEEE;--muted:#93A5A6;--rule:#243031;--accent:#4CCFC0;
...
.card .top{display:flex;justify-content:space-between;gap:8px;font:12px ui-monospace,Menlo,monospace;color:var(--muted)}
...
.card .foot{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:3px}
...
function render(){
  ...
  for(const [key,label] of COLS){
    ...
    for(const s of items){
      const d=document.createElement('div');d.className='card';d.style.setProperty('--c','var(--'+key+')');
      ...
      d.innerHTML='<div class="top"><b>'+esc(s.repo||'?')+(s.branch?' · '+esc(s.branch):'')+'</b><span>'+ago(s.lastEventAt)+'</span></div>'
       ...
       +'<div class="foot"><span class="chip">'+esc(s.surface)+'</span>'+(s.permissionMode?'<span class="chip">'+esc(s.permissionMode)+'</span>':'')+'<span class="chip">'+s.turnCount+' turns</span>'
       +'<button data-cmd="'+esc(resume)+'">copy resume</button>'+(s.issue&&s.issue.url?'<a href="'+esc(s.issue.url)+'" target="_blank">#'+s.issue.number+'</a>':'')+'</div>';
      col.appendChild(d);
    }
    b.appendChild(col);
  }
